...
$templatefile= 'C:\Users\roopsa.d\Downloads\ARM\EventHub\EventHubTemplate.json'   #path to template file
$parameterfile= 'C:\Users\roopsa.d\Downloads\ARM\EventHub\EventHubParameters.json' #path to parameter file
$rg= 'RG-IND-TEST-PAAS'                                                    #resource group name
#$loc= 'central india'                                               #location
$name= 'test'                                                 #name
$sku= 'Basic' 


#if resource exists, print that it exists and do not create new resource
    if((Get-AzResource -Name $name -ResourceType Microsoft.EventHub/namespaces).Name -eq $name){
        ...
        # Do resource group deployment if resource group does not exist by creating new resource group and then create the resource
         ...
         New-AzResourceGroupDeployment -ResourceGroupName $rg -Name $name -TemplateFile $templatefile -TemplateParameterFile $parameterfile
