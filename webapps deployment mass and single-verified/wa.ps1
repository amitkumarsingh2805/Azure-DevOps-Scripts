...
#######################
$templatefile= 'C:\Users\ritik.d\Desktop\web-app\wa-template.json'    	    #path to template file
$parameterfile= 'C:\Users\ritik.d\Desktop\web-app\wa-parameters.json' 	    #path to parameter file
$rg= 'sam1'                                                                  #resource group name
...
#New-AzResourceGroup -Name testabc123 -Location northcentralus(if new resource group is required)
    ...
    if((Get-AzurermResource -Name $name -ResourceType Microsoft.Web/serverfarms).Name -eq $name){
        ...
        Write-Host ("Resource group does not exist. Creating new resource group: " +$rg)
         ...
         New-AzurermResourceGroupDeployment -ResourceGroupName $rg -sku $sku -siteName $siteName -hostingPlanName $hostingPlanName -TemplateFile $templatefile -TemplateParameterFile $parameterfile
